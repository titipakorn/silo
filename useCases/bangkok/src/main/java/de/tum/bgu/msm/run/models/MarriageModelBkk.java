...
import de.tum.bgu.msm.run.data.person.Nationality;
 ...
 **/
public class MarriageModelBkk extends AbstractModel implements MarriageModel {

  private final static Logger logger = Logger.getLogger(MarriageModelBkk.class);
  ...
  public MarriageModelBkk(DataContainer dataContainer, MovesModelImpl movesModel,
  ...
  private Person findPartner(MarriageMarket market, Person person) {
    ...
    Nationality personRace = ((PersonBkk) person).getNationality();
    float sum = 0;
    for (Person pp : possiblePartners) {
      float prob;
      PersonBkk p = ((PersonBkk) pp);
  ...
  private void movePerson(Person person1, Household moveTo) {
    ...
    PersonBkk pp = (PersonBkk) person1;
    if (!moveTo.equals(household1)) {
      householdDataManager.removePersonFromHousehold(person1);
      householdDataManager.addPersonToHousehold(pp, moveTo);
  ...
  private void moveRemainingChildren(Household oldHh, Household newHh) {
    ...
    for (Person person : remainingPersons) {
      PersonBkk pp = (PersonBkk) person;
      householdDataManager.removePersonFromHousehold(person);
      householdDataManager.addPersonToHousehold(pp, newHh);
