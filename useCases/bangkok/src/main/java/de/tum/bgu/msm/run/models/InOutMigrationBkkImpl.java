 ...
 **/
public class InOutMigrationBkkImpl extends AbstractModel implements InOutMigration {

  private final static Logger logger = Logger.getLogger(InOutMigrationBkkImpl.class);
  ...
  public InOutMigrationBkkImpl(DataContainer dataContainer, EmploymentModel employment,
  ...
  private boolean inmigrateHh(Household hh) {
    ...
    if (newDdId > 0) {
      ...
      for (Person person : hh.getPersons().values()) {
        PersonBkk p = (PersonBkk) person;
        householdDataManager.addPerson(p);
      }
      return true;
    } else {
